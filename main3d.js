/* main3d.js — Ultra refined: cloud/star orbs (plane textures), sunrise→sunset, improved cinematic motion
   Requires:
   - three.min.js r149
   - simplex-noise 2.4.0
...
(() => {
  ...
  // quick mobile fallback
  if (isMobile) {
    loaderEl?.classList.add('hidden');
    console.log('Mobile device detected — disabling heavy scene.');
    return;
  }
  ...
  // background plane (horizon image)
  ...
  const hemi = new THREE.HemisphereLight(0xfff4ea, 0x2b2b3a, 0.56); scene.add(hemi);
  const sun = new THREE.DirectionalLight(0xffe7c6, 1.0); sun.position.set(40, 70, -40); scene.add(sun);
  const ambient = new THREE.AmbientLight(0xffffff, 0.12); scene.add(ambient);

  // dunes geometry
  const planeSize = 900, segX = 280, segY = 160;
  ...
  const duneMat = new THREE.MeshPhysicalMaterial({
    color: 0xdbcdb5, roughness: 0.92, metalness: 0.02, clearcoat: 0.06, sheen: 0.14, sheenColor: 0xefe2d1
  });
  const dunes = new THREE.Mesh(geom, duneMat); dunes.position.y = -7.0; scene.add(dunes);
  ...
  cone.position.set(0, 80, -120); cone.rotateX(Math.PI); scene.add(cone);

  // noise
  const simplex = new SimplexNoise(Math.random);

  // canvas texture creators for clouds & stars
  function makeCloudTexture(size = 1024) {
    const c = document.createElement('canvas'); c.width = c.height = size; const ctx = c.getContext('2d');
    ctx.clearRect(0, 0, size, size);
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ctx.arc(size * 0.32, size * 0.46, size * 0.25, 0, Math.PI * 2);
    ctx.arc(size * 0.54, size * 0.38, size * 0.28, 0, Math.PI * 2);
    ctx.arc(size * 0.7, size * 0.5, size * 0.2, 0, Math.PI * 2);
    ctx.closePath(); ctx.fill();
    const grad = ctx.createRadialGradient(size * 0.45, size * 0.34, 10, size * 0.6, size * 0.5, size * 0.45);
    grad.addColorStop(0, 'rgba(255,255,255,0.95)'); grad.addColorStop(0.6, 'rgba(255,255,255,0.28)'); grad.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.fillStyle = grad; ctx.fill();
    const tex = new THREE.CanvasTexture(c); tex.needsUpdate = true; return tex;
  }
  function makeStarTexture(size = 1024) {
    const c = document.createElement('canvas'); c.width = c.height = size; const ctx = c.getContext('2d');
    ctx.clearRect(0, 0, size, size); ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ...
    grad.addColorStop(0, 'rgba(255,255,255,0.95)'); grad.addColorStop(0.6, 'rgba(255,255,255,0.2)'); grad.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.fillStyle = grad; ctx.fill();
    const tex = new THREE.CanvasTexture(c); tex.needsUpdate = true; return tex;
  }
  ...
  function makeOrbMaterial(mapTex, metal = true) {
    const mat = new THREE.MeshPhysicalMaterial({
      map: mapTex, transparent: true, alphaTest: 0.01, side: THREE.DoubleSide,
      metalness: metal ? 0.9 : 0.02, roughness: metal ? 0.08 : 0.18, clearcoat: metal ? 1.0 : 0.0, clearcoatRoughness: metal ? 0.02 : 0.5
    });
  ...
  // textures & mats
  const cloudTex = makeCloudTexture(1024);
  const starTex = makeStarTexture(1024);
  const cloudMat = makeOrbMaterial(cloudTex, true);
  const starMat = makeOrbMaterial(starTex, true);

  // spawn orbs (cloud + star planes)
  const orbPositions = [
    [-36, 28, -140], [18, 36, -200], [42, 22, -120], [-72, 30, -240],
    [10, 26, -300], [86, 36, -360], [-130, 42, -420], [128, 30, -180]
  ];
  orbPositions.forEach((p, i) => {
    const size = 12 + Math.random() * 18;
    const geo = new THREE.PlaneGeometry(size, size * 0.62);
    const mat = (i % 4 === 0) ? starMat.clone() : cloudMat.clone();
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.set(p[0], p[1], p[2]);
    mesh.userData = { baseX: p[0], baseY: p[1], drift: 0.006 + Math.random() * 0.012, wobble: 0.6 + Math.random() * 1.2 };
    mesh.rotation.y = Math.PI * 0.18 * (Math.random() - 0.4);
    orbGroup.add(mesh);
  });

  // butterflies (flat chrome planes)
  ...
  // dust points
  const dustCount = 420; const dustGeo = new THREE.BufferGeometry(); const dustArr = new Float32Array(dustCount * 3);
  for (let i = 0; i < dustCount; i++) { dustArr[i * 3 + 0] = (Math.random() - 0.5) * 900; dustArr[i * 3 + 1] = Math.random() * 40 + 2; dustArr[i * 3 + 2] = -Math.random() * 800; }
  ...
  // day param and pointer
  let dayT = 0.02; let autoT = 0; const pointer = { x: 0, y: 0 };
  window.addEventListener('mousemove', e => { pointer.x = (e.clientX / innerWidth) * 2 - 1; pointer.y = (e.clientY / innerHeight) * 2 - 1; }, { passive: true });
  window.addEventListener('scroll', () => { const dh = document.documentElement.scrollHeight - window.innerHeight; const s = dh > 0 ? window.scrollY / dh : 0; dayT = Math.min(1, Math.max(0, s)); }, { passive: true });
  ...
  function animate() {
    ...
    // orb motion (layered noise) — calmer, cinematic
    orbGroup.children.forEach((o, idx) => {
      const ud = o.userData;
      const dx = simplex.noise3D((ud.baseX + idx) * 0.007 + t * 0.02, idx * 0.13, t * 0.004) * 8;
      const dz = simplex.noise3D((ud.baseX + idx) * 0.011 + t * 0.01, t * 0.006, idx) * 6;
      o.position.x = ud.baseX + dx;
      o.position.z += dz * 0.06;
      o.position.y -= ud.drift * 0.12; // slower descent
      if (o.position.y < -9) {
      ...
      o.rotation.x = Math.sin(t * 0.06 + idx * 0.22) * 0.03;
    });
    ...
    // camera parallax (pointer + slow orbit)
    ...
    // sunrise → sunset blending (scroll-driven + auto drift)
  ...
  // intersection observer for panels (mirage float)
  ...
  // pointer-follow highlight for orb buttons
